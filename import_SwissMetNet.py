...
targetDir = 'D:/Applications/MaJ_donnees_air/COPIED_FILES/'
fileList = os.listdir(targetDir + 'swissMetNet')
nbFile = len(fileList)
k = 0
for filename in fileList:
    sqlCheckImport = "select count(*) as countItem from stations_air.import_log where filename = '" + filename.replace('.','_') + "';"
    ...
    if isAlreadyLoaded == 0:
        ...
        sqlLog = "insert into stations_air.import_log (idobj, filename, importdatetime, sucess) values ('" + idobjLog + "','" + filename.replace('.','_') + "','" + currentDateTime + "', 'true');"
        cur.execute(sqlLog)
    else:
